...
- Ads, search and Maps: say the brand runs or does not run paid ads only from the "Meta Ad Library" and "Google Ads Transparency Center" checks, and only for the platform and country the check names. A "Brave Search" rank is Brave's result: never write it as a Google ranking. Google rankings, ads on Google searches and the Google map come only from the "Google:" checks, which read the first page of results in one country on one day: say "not on the first page", never "not on Google", and name the search. Ads shown when people search the brand's name come from other advertisers unless the check says they are the client's. A Google Maps listing, rating or owner replies come only from the "Google Maps" checks. Facebook comment replies cover only the visible comments the check counted; say so instead of generalising. A check that could not read its source is unknown, not a problem.
</blueprint_rules>
