...
export function deliveryPage({ slug, p, state }) {
  ...
  const usageTable = usage.total.runs
    ? `${table(['Step', ['Runs', 'num'], ['Time', 'num'], ['Claude usage', 'num']], [...usage.steps.map((u) => `<tr><td>${esc(u.label)} <span class="muted small">${esc(u.models.join(', '))}</span>${u.failed ? ` <span class="tag tag-quiet">${u.failed} failed</span>` : ''}${u.redone ? ` <span class="tag tag-quiet" title="The answer did not pass the code checks and was written again">${u.redone} redone</span>` : ''}</td><td class="num">${u.runs}</td><td class="num">${esc(duration(u.durationMs))}</td><td class="num">${usd(u.costUsd)}${u.unknownCost ? '<abbr title="Some runs did not report a cost">*</abbr>' : ''}</td></tr>`), `<tr class="row-summary"><td>Total for this proposal</td><td class="num">${usage.total.runs}</td><td class="num">${esc(duration(usage.total.durationMs))}</td><td class="num"><b>${usd(usage.total.costUsd)}</b></td></tr>`])}
