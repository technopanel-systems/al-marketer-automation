...
export function social({ slug, p, state, msg }) {
  ...
  <form class="card" method="post" action="/c/${slug}/social"><h3>2 · Capture the profiles</h3>
    <p class="small muted"><b>No login is used.</b> LinkedIn company pages, Facebook pages, X, Instagram, TikTok and YouTube are read from their public pages, a few requests per profile at a slow pace. If a platform refuses for now, "Try automatic again" later, or use <b>Type numbers</b>. ${t.tasks.some((x) => x.method === 'assisted') ? 'Optional fallback — <b>research browser</b>: a separate Chrome window; open the profile, scroll until the panel says 3 months are covered, then press <b>Done — save</b>. Nothing is posted, liked or followed.' : ''}</p>
