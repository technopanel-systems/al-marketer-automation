...
| Research | Website audit + meeting notes (Haiku; Sonnet for long reports) together; then client social profiles, competitor search (Sonnet + web search), research teams (Sonnet ×3), business signals, **Google search** (brand name and buyer searches: rank, others on page 1, ads, map; no account, nothing saved) and **ads, search & Maps checks** (Meta Ad Library, Brave Search, Google Maps, Google Ads Transparency Center, Facebook comment replies) side by side; then the business analyst (Sonnet) and the client record | Confirm competitors (while research continues) · important questions only if something is missing · optional checks never hold anything up |
